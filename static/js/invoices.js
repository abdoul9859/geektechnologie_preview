        ...
        setTimeout(() => {
            ...
            try {
                ...
                if (raw) {
                    ...
                    waitForProductsLoaded().then(() => preloadPrefilledInvoiceFromQuotation(data));
                }
            } catch (e) {}
        }, 300);
        return;
...
function setupEventListeners() {
    ...
    document.getElementById('clientFilter')?.addEventListener('input', debounce(filterInvoices, 300));
    document.getElementById('dateFromFilter')?.addEventListener('change', filterInvoices);
    document.getElementById('dateToFilter')?.addEventListener('change', filterInvoices);
    document.getElementById('invoiceSearch')?.addEventListener('input', debounce(filterInvoices, 250));
...
function filterInvoices() {
    ...
    filteredInvoices = invoices.filter(invoice => {
        ...
        // Filtre par recherche (ID ou numéro)
        if (search) {
            const idStr = String(invoice.invoice_id || '').toLowerCase();
            const numStr = String(invoice.invoice_number || '').toLowerCase();
            if (!(idStr.includes(search) || numStr.includes(search))) {
                return false;
            }
        }

        // Filtre par date (robuste si date manquante)
        try {
            const rawDate = invoice.date || invoice.invoice_date || invoice.created_at || null;
            let invoiceDate = '';
            if (rawDate) {
                const d = new Date(rawDate);
                if (!Number.isNaN(d.getTime())) invoiceDate = d.toISOString().split('T')[0];
            }
            if (dateFromFilter && invoiceDate && invoiceDate < dateFromFilter) {
                return false;
            }
            if (dateToFilter && invoiceDate && invoiceDate > dateToFilter) {
                return false;
            }
        } catch (e) {
            // ignore invalid date when filtering by dates
        }
...
function preloadPrefilledInvoiceFromQuotation(prefill) {
    openInvoiceModal();
...
async function loadInvoiceDetail(invoiceId) {
    ...
    `).join('');
    // Extract serials meta from notes if present
...
async function savePayment() {
    try {
        ...
        await axios.post(`/api/invoices/${paymentData.invoice_id}/payments`, {
